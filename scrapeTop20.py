...
codes += ['10229611','10229612']
events += ['Men110mHurdles','Men400mHurdles']
...
codes += ['10229522','10229523']
events += ['Women100mHurdles','Women400mHurdles']
...
for c in range(len(codes)):
    ...
    for result in line.split('"f1-result-column">')[2:]:
        if events[c][-1] == 'm' or 'Hurdles' in events[c]:
            time = result.split('<')[0]
            time = time.replace('h','')
            if time.find(':') != -1:
...
with open('top20ConvFactors.csv','w') as csvfile:
    ...
    for i in range(len(codes)/2):
        ...
        elif 'Hurdles' in events[i]:
            speed = int(events[i].replace('Men','').replace('Women','').replace('Hurdles','')[:-1]) / avbestscores[i]
